...
class JdSpider(scrapy.Spider):
    ...
    def parse_book_list(self, response, item): # 解析列表页
        item = response.meta["item"]
        li_list = response.xpath("//div[@class='plist']/ul/li")
        for li in li_list:
            ...
            item["book_publish_date"] = li.xpath(".//span[@class='p-bli-date']/text()").extract_first().strip()
            item["book_sku"] = lix.xpath("./div/@data-sku").extract_first()
            yield scrapy.Request(
    ...
    def parse_book_price(self, response, item):
        item = response.meta["item"]
