...
# 前端传递的参数: 地址id, (addr_id) 支付方式(pay_method), 商品id(sku_ids)
class OrderCommitView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if not all([addr_id, pay_method, sku_ids]):
            return JsonResponse({'ret': 1, 'errmsg': '参数不完整'})
        # 校验支付方式
        if pay_method not in OrderInfo.PAY_METHODS.keys():
            return JsonResponse({'ret': 2, 'errmsg': '非法的支付方式'})
        ...
        # todo: 创建订单核心业务
        # 用户每下一个单, 就需要向df_order_info表中加入一条记录, 用户的订单中有几种商品, 就需要向df_order_goods表中加几条记录
        # 组织参数
        # 订单id: 20170403121343+用户id
        ...
        try:
            ...
            # todo: 用户的订单中有几种商品, 需要向df_order_goods表中加入几条记录(订单表 和 商品表 是多对多的关系)
            conn = get_redis_connection('default')
            cart_key = 'cart_%d' % user.id
            ...
            for sku_id in sku_ids:
                # 获取商品的信息
                try:
                    # select_for_update()加锁(悲观锁): select * from df_goods_sku where id=sku_id for update;
        ...
        # todo: 清除用户购物车中对应的记录
        conn.hdel(cart_key, *sku_ids)  # 传过来一个列表[1,2,3], 会拆包成 conn.hdel(cart_key, 1, 2, 3)
